...
def train_with_grasp(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        # forward
        with amp.autocast():
            pred, target, loss, loss_dict = model(image, depth, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask)
        ...
        # metric
        iou, pr5 = trainMetricGPU(ins_mask_pred, ins_mask_target, 0.5, 0.5)
...
def validate_with_grasp(val_loader, model, epoch, args):
    ...
    for data in pbar:
        ...
        pred, target = model(image, depth, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask)
        ...
        for idx in range(ins_mask_preds.shape[0]):
            ...
            for i in range(len(num_grasps)):
                ...
                if args.visualize:
                    ...
                    # visualization(img, ins_mask_target, (grasp_qua_mask_target, grasp_angle_gt, grasp_wid_mask_target), grasp_rec_gt, sent, save_path=os.path.join("./results", args.exp_name, f"gt_{cnt}_{num_g}_grasps.png"))
    J_index = [0, 0]
    for i in range(len(num_grasps)):
        J_index[i] = num_correct_grasps[i]/num_total_grasps[i]
