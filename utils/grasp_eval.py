...
def overlay_davis(image, mask, colors=[[0, 0, 0], [0, 255, 255]], cscale=1, alpha=0.4):
...
def visualization(img, mask, grasp_masks, grasps, text, save_path=None):
    ...
    for rect in grasps:
        ...
        box = np.intp(box)
        ptA, ptB, ptC, ptD = [tuple(map(int, pt.tolist())) for pt in box]
    ...
    msk_img = overlay_davis(img, mask)
