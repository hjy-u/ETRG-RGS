...
class OCIDVLGDataset(Dataset):
    ...
    def preprocess(self, data):
        ...
        ins_mask = cv2.warpAffine(ins_mask,
                                    ...
                                    flags=cv2.INTER_LINEAR,
                                    borderValue=0.)
        scaled_depth = (depth * 255 / (depth.max() + 1e-4)).astype(np.uint8)
        resize_depth = cv2.warpAffine(scaled_depth,
                                  mat,
                                  self.input_size,
                                  flags=cv2.INTER_LINEAR,
                                  borderValue=0.)
        depth = resize_depth.astype(np.float32) / 255 * (depth.max() + 1e-4)
        ...
        # # normalize depth, mean = 0.5, std = 0.25
        depth = 1- (depth) / (depth.max() + 1e-5)
        depth[np.isnan(depth)] = 0
    ...
    def get_depth_from_path(self, path):
        depth = cv2.imread(path, cv2.IMREAD_UNCHANGED).astype(np.float32) / 1000. # mm -> m
        # depth = depth / (np.max(depth) + 1e-4)
        return depth
